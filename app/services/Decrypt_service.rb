class DecryptService
    ...
    def make_equation
        @correspondences.each do |hash|
            ...
            begin
                ...
                DecryptTry.create(decrypt_id: decrypt.id, error_log: e.message.to_s, correspondence: hash)
                puts e
            end
        end
    end
    ...
    def set_correspondences
        ...
        @correspondences =[
            ...
            {mfc.first => mal.second, mfc.second => mal.first},
            {mfc.first => mal.first, mfc.second => mal.third},
            {mfc.first => mal.first, mfc.second => mal.fourth},
        ] 
    end

    private

    def inverse(a, m=27) # compute a^-1 mod m if possible
